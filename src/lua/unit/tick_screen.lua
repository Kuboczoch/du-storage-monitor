...
function cobaltiteStock(percent)
    local cobaltiteType = oreType[1] ..coma
    local cobaltiteOk = ""
    if percent <=25 then return
    cobaltiteType
    else return
...
function cryoliteStock(percent)
    local cryoliteType = oreType[2] ..coma
...
function kolbeckiteStock(percent)
    local kolbeckiteType = oreType[3] ..coma
    local kolbeckiteOk = ""
    if percent <=25 then return
    kolbeckiteType
    else return
...
oreType = {[1]="Cobaltite",[2]="Cryolite",[3]="Kolbeckite",[4]="Gold nuggets"}
oreWeight = {[1]=6.33,[2]=2.95,[3]=2.37,[4]=19.30}

local maxWitdh = 46.5

maxContainerVolumeTier4 = 153600 --export: Max Tier4 Container Volume

-- Container_1
local containerMass_cobaltite = math.ceil(container_cobaltite.getItemsMass()/oreWeight[1])
local percentCobaltite = math.ceil((containerMass_cobaltite/maxContainerVolumeTier4)*100)
local cobaltiteStatus = resourcesStatus(percentCobaltite)
local cobaltiteFactor = maxWitdh/maxContainerVolumeTier4
...
local percentCryolite = math.ceil((containerMass_cryolite/maxContainerVolumeTier4)*100)
local cryoliteStatus = resourcesStatus(percentCryolite)
local cryoliteFactor = maxWitdh/maxContainerVolumeTier4
...
local percentKolbeckite = math.ceil((containerMass_kolbeckite/maxContainerVolumeTier4)*100)
local kolbeckiteStatus = resourcesStatus(percentKolbeckite)
local kolbeckiteFactor = maxWitdh/maxContainerVolumeTier4
...
local containerVolumeKolbeckite = converUnit(containerMass_kolbeckite)

-- Container_4
local containerMass_gold_nuggets = math.ceil(container_gold_nuggets.getItemsMass()/oreWeight[4])
local percentGold_nuggets = math.ceil((containerMass_gold_nuggets/maxContainerVolumeTier4)*100)
local gold_nuggetsStatus = resourcesStatus(percentGold_nuggets)
local gold_nuggetsFactor = maxWitdh/maxContainerVolumeTier4
...
local containerVolumeGold_nuggets = converUnit(containerMass_gold_nuggets)
